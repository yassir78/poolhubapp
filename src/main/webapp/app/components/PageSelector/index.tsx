...
const PageSelector = () => {
  ...
  const pageNumber = (page: number | string, selected: boolean = false):JSX.Element => {
    return <a
      href="#"
      className={`${selected ? "font-bold" : ""}  relative hidden items-center bg-white px-4 py-2 text-sm font-md text-textGray hover:bg-octonary focus:z-20 md:inline-flex`}
    >
      {page}
    </a>
  }
